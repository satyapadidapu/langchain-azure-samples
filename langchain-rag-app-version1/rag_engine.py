...
def get_embeddings():
    """Create Azure OpenAI Embeddings from environment variables."""
...
def process_files(file_paths):
    """Process uploaded files: split documents into chunks and build a FAISS vector store.

    Args:
        file_paths: List of file paths to process.

    Returns:
        Tuple of (vector_store, image_data_list, file_names).
        vector_store: FAISS index (or None if no documents).
        image_data_list: List of (base64_str, mime_type, filename) for images.
    ...
    for fp in file_paths:
        ...
        if is_image(fp):
            ...
            image_data.append((b64, mime, name))
        else:
